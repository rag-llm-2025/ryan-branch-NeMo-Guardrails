...
class GPT4OWrapper:
    ...
    def stream_chat_completion(
        ...
        try:
            ...
            error_message = str(e)
            if "400" in error_message:
                error_data = {"error": "请求参数错误", "status_code": "400"}
            elif "401" in error_message:
                error_data = {"error": "API认证失败", "status_code": "401"}
            elif "403" in error_message:
                error_data = {"error": "访问被拒绝", "status_code": "403"}
            elif "500" in error_message:
                error_data = {"error": "服务器内部错误", "status_code": "500"}
            else:
                error_data = {"error": "网络连接错误", "status_code": "unknown"}
            print(f"error_message: {error_message}, error_data: {error_data}")
            yield error_data
    ...
    def batch_process(
        ...
        with open(input_file, "r") as f:
            for index, line in enumerate(f, 1):
                ...
                print(f"{index}. User: {text}\n")
                # print(f"Processing {index}: {text[:50]}...")
                ...
                # print(f"response: {content} (latency={cost_time})")
    ...
    def _process_response(self, response: Dict) -> Union[str, Dict]:
        ...
        if "error" in response and response["error"].get("code") == "content_filter":
            print(f"内容过滤: {response['error']['message']}")
    ...
    def interactive_mode(
        ...
        while True:
            ...
            content = self._process_response(response)
            print(f"{index}. Bot: {content} (latency={cost_time:.2f}ms)")
            print(
                "-----------------------------------------------------------------\n"
            )
            index += 1
